...
| Frozen-Qwen decoder memory injection | RETRY: RAW PREFIX FAILED | `python scripts/smoke_temporal_memory_qwen_decoder.py --model-path /root/local-checkpoints/Qwen3-VL-8B-Instruct` | With a successful 79.88% memory warmup, naive continuous-prefix injection into a fully frozen decoder still stayed near chance: normal 28.52%, shuffled 25.78%, zero-memory 24.41%. This isolates the failure to decoder/read alignment. The next retry uses an explicit lightweight decoder-side cross-attention adapter while keeping Qwen frozen and retaining shuffled/zero controls. |
