...
| One-step Trainer/update smoke | READY | `bash scripts/smoke_train_step_1gpu.sh` | Single-GPU safety harness uses stateless SGD for exactly one update and suppresses full-model checkpoint saving. This validates Trainer/update plumbing only; it is not the paper's AdamW + multi-GPU ZeRO configuration. |
