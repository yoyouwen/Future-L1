...
| Frozen-Qwen visual-feature memory smoke | READY | `python scripts/smoke_temporal_memory_qwen_features.py --model-path /root/local-checkpoints/Qwen3-VL-8B-Instruct` | Renders 32-frame controlled sequences, extracts and caches features with a frozen Qwen3-VL vision encoder, and repeats the equal-16-token memory/uniform/shuffled A/B. Only the lightweight memory/control heads train. |
