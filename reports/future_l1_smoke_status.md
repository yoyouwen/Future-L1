...
| Temporal latent-memory mechanism smoke | READY | `python scripts/smoke_temporal_latent_memory.py` | Controlled feature-level A/B test trains hierarchical clip compression plus causal temporal memory against an equal-global-token uniform baseline, then evaluates a shuffled-memory temporal-order ablation. This tests whether the mechanism is learnable before integrating Qwen video features. |
