...
| Frozen-Qwen decoder memory injection | READY | `python scripts/smoke_temporal_memory_qwen_decoder.py --model-path /root/local-checkpoints/Qwen3-VL-8B-Instruct` | Maps 16 continuous temporal-memory tokens into Qwen's 4096-d input space and trains them through the fully frozen decoder. Constrained LM-head A/B/C/D accuracy is compared with shuffled and zero-memory controls; no checkpoint is saved. |
