...
| Dataset collator smoke | RETRY: CHAT TEMPLATE | `python scripts/smoke_dataset_sft.py --data-path /path/to/tiny.json --model-path /path/to/Qwen3-VL-8B-Instruct` | Dataset normalization and image tensors passed, but the base checkpoint template omitted assistant images, yielding zero latent tokens. Smoke scripts now explicitly load the repo's Future-L1 chat template and assert positive/equal latent-token and image-out-mask counts. |
| SFT forward/loss smoke | RETRY: RUNPOD I/O | `python scripts/smoke_forward_sft.py --model-path /path/to/Qwen3-VL-8B-Instruct --data-path /path/to/tiny.json` | First RunPod attempt loaded shards, then blocked in `model.to(cuda)` with `folio_wait_bit_common` against the network-volume checkpoint. The smoke now loads shards directly onto the target GPU via a one-device `device_map`; retry pending. |
