...
| Frozen-Qwen decoder memory injection | PASS: CROSS-ATTENTION; FAIL: RAW PREFIX | `python scripts/smoke_temporal_memory_qwen_decoder.py --model-path /root/local-checkpoints/Qwen3-VL-8B-Instruct --injection-mode cross_attention` | After a 79.88% memory warmup, explicit decoder-side cross-attention reached 86.72% constrained LM-head accuracy. Shuffling memory reduced accuracy to 34.70% and zero memory to 24.41%, giving +52.02 pp order gain and +62.30 pp gain over zero. In contrast, naive prefix injection into the fully frozen decoder reached only 28.52%. Controlled rendered prototypes and four-way constrained decoding remain limitations. |
